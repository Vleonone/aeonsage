...
function resolveConfiguredModelRaw(cfg: AeonSageConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: AeonSageConfig): string[] {
...
export function applyPrimaryModel(cfg: AeonSageConfig, model: string): AeonSageConfig {
...
export function applyModelAllowlist(cfg: AeonSageConfig, models: string[]): AeonSageConfig {
  const defaults = cfg.agents?.defaults;
