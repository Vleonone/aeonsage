...
homepage: https://docs.aeonsage.org/hooks#command-logger
metadata:
  {
    "aeonsage":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.aeonsage/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.aeonsage/logs/commands.log ~/.aeonsage/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/aeonsage`:
   ```
   /home/username/.aeonsage/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
