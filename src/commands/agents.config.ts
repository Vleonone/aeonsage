...
export function listAgentEntries(cfg: AeonSageConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AeonSageConfig, agentId: string) {
...
function resolveAgentModel(cfg: AeonSageConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AeonSageConfig): AgentSummary[] {
