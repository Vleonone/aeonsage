...
declare const __AEONSAGE_VERSION__: string;

const BUNDLED_VERSION =
  (typeof __AEONSAGE_VERSION__ === "string" && __AEONSAGE_VERSION__) ||
  process.env.AEONSAGE_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `aeonsage --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.AEONSAGE_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.AEONSAGE_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
...
void main().catch((err) => {
  console.error(
    "[aeonsage] Gateway daemon failed:",
