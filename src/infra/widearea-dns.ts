...
export const WIDE_AREA_DISCOVERY_DOMAIN = "aeonsage.internal.";
export const WIDE_AREA_ZONE_FILENAME = "aeonsage.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aeonsage-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "aeonsage";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "aeonsage");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "aeonsage-gw");
  ...
  records.push(`_aeonsage-gw._tcp IN PTR ${instanceLabel}._aeonsage-gw._tcp`);
  records.push(`${instanceLabel}._aeonsage-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._aeonsage-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; aeonsage-content-hash: ${contentHash}\n${contentBody}`;
}
