...
import type { AeonSagePluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: AeonSagePluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: AeonSagePluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
