...
function setWhatsAppDmPolicy(cfg: AeonSageConfig, dmPolicy: DmPolicy): AeonSageConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: AeonSageConfig, allowFrom?: string[]): AeonSageConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: AeonSageConfig, selfChatMode: boolean): AeonSageConfig {
...
async function detectWhatsAppLinked(cfg: AeonSageConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so AeonSage can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for AeonSage" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("aeonsage channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
