...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "aeonsage.sandbox=1");
  args.push("--label", `aeonsage.sessionKey=${params.scopeKey}`);
  args.push("--label", `aeonsage.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `aeonsage.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "aeonsage.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`aeonsage sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`aeonsage sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("aeonsage sandbox recreate --all");
}
