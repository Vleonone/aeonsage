...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aeonsage-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      AEONSAGE_NO_RESPAWN: "1",
      AEONSAGE_STATE_DIR: stateDir,
      AEONSAGE_SKIP_CHANNELS: "1",
      AEONSAGE_SKIP_GMAIL_WATCHER: "1",
      AEONSAGE_SKIP_CRON: "1",
      AEONSAGE_SKIP_BROWSER_CONTROL_SERVER: "1",
      AEONSAGE_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "aeonsage-entry-bootstrap.mjs");
