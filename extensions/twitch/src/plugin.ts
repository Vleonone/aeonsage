/**
 * Twitch channel plugin for AeonSage.
 *
...
import { buildChannelConfigSchema } from "aeonsage/plugin-sdk";
 ...
 * for AeonSage. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AeonSageConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AeonSageConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AeonSageConfig): boolean => {
