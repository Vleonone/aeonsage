...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of aeonsage")).toBe("friends-of-aeonsage");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "Vleonone", "Friends of aeonsage"],
    ...
    expect(allowListMatches(allow, { name: "Vleonone" })).toBe(true);
    expect(allowListMatches(allow, { name: "friends-of-aeonsage" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-aeonsage" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of aeonsage"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-aeonsage");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-aeonsage": { slug: "friends-of-aeonsage" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of aeonsage"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["aeonsage-dm"],
        channelId: "1",
        channelName: "aeonsage DM",
        channelSlug: "aeonsage-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
