...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/aeonsage/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/aeonsage");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/aeonsage/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AEONSAGE_CONTROL_UI_BASE_PATH__ = "/aeonsage";
    const app = mountApp("/aeonsage/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/aeonsage");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/aeonsage/sessions");
  });
