...
export function listIMessageAccountIds(cfg: AeonSageConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: AeonSageConfig): string {
...
function mergeIMessageAccountConfig(cfg: AeonSageConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: AeonSageConfig): ResolvedIMessageAccount[] {
