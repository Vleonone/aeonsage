 ...
 * This enables discovery of skills in nested directory structures like:
 * MASTER_SKILL_PACK_COMPLETE/SENSEI/skill-name/SKILL.md
 */
function findSkillDirsRecursive(dir: string, maxDepth = 5): string[] {
...
function loadSkillEntries(
  ...
  /**
   * Recursively load skills from a directory tree.
   * This finds all nested SKILL.md files and loads them.
   */
  const loadSkillsRecursive = (params: { dir: string; source: string }): Skill[] => {
    if (!fs.existsSync(params.dir)) return [];

    const skillDirs = findSkillDirsRecursive(params.dir);
    const allSkills: Skill[] = [];

    // For each directory containing SKILL.md, load skills from its parent
    const parentDirs = new Set<string>();
    for (const skillDir of skillDirs) {
      const parentDir = path.dirname(skillDir);
      if (parentDir !== params.dir) {
        parentDirs.add(parentDir);
      }
    }

    // First load from the main directory
    allSkills.push(...loadSkills(params));

    // Then load from each discovered parent directory
    for (const parentDir of parentDirs) {
      const skills = loadSkills({ dir: parentDir, source: params.source });
  ...
  // Use recursive loading for bundled skills to discover nested skill packs
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkillsRecursive({
      dir: resolved,
      source: "aeonsage-extra",
    });
  });
  const managedSkills = loadSkillsRecursive({
    dir: managedSkillsDir,
    source: "aeonsage-managed",
  });
  // Use recursive loading for workspace skills to find all nested skills
  const workspaceSkills = loadSkillsRecursive({
    dir: workspaceSkillsDir,
    source: "aeonsage-workspace",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      ...
      skill,
      frontmatter,
      metadata: resolveAeonSageMetadata(frontmatter),
