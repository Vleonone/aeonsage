...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".aeonsage", "disable-launchagent");
...
function hasConfigGatewayCreds(cfg: AeonSageConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("AEONSAGE_GATEWAY_TOKEN");
  const envPassword = await getenv("AEONSAGE_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `AEONSAGE_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `AEONSAGE_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv AEONSAGE_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv AEONSAGE_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
