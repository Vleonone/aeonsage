...
import { buildWorkspaceSkillSnapshot, loadWorkspaceSkillEntries } from "../agents/skills.js";
import { autoTriggerSkills, generateSkillTriggerPrompt } from "../agents/skills/auto-trigger.js";
...
export async function agentCommand(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("aeonsage agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  try {
    ...
    // ✨ Skill Auto-Trigger: Analyze message intent and match relevant skills
    ...
    const triggeredSkills = autoTriggerSkills(body, skillEntries, {
      enabled: true, // Auto-trigger enabled by default
      minConfidence: 0.6,
      maxSkills: 3,
    });
    ...
    // [SOVEREIGN COGNITIVE ROUTER]
    // Intercept default routing with localized intelligence
    let provider = defaultProvider;
    let model = defaultModel;

    try {
      // Only route if explicit model override is NOT present (let user force model if they want)
      if (!sessionEntry?.modelOverride && !opts.model) {
        ...
        // Log routing decision (visible in verbose mode or logs)
        if (opts.verbose) {
          console.log(`[CognitiveRouter] Tier: ${routeResult.tier} | Model: ${routeResult.model}`);
        }
      ...
      // Fail open: strict reliability. If router crashes, fall back to default dumb routing.
    }
    ...
    try {
      ...
      // ✨ Combine extra system prompt + skill trigger prompt
      const combinedSystemPrompt = [opts.extraSystemPrompt, skillTriggerHint]
        .filter(Boolean)
        .join("\n\n");
        ...
        run: (providerOverride, modelOverride) => {
          if (isCliProvider(providerOverride, cfg)) {
            ...
            return runCliAgent({
              ...
              timeoutMs,
              runId,
              extraSystemPrompt: combinedSystemPrompt || undefined,
              cliSessionId,
              images: opts.images,
