...
function shouldSetOpenAICodexModel(model?: string): boolean {
  ...
  // Recognized Codex model IDs that should not be overridden.
  if (normalized === "gpt-5.3-codex" || normalized === "gpt-5.3") return false;
...
export function applyOpenAICodexModelDefault(cfg: AeonSageConfig): {
  next: AeonSageConfig;
  changed: boolean;
} {
