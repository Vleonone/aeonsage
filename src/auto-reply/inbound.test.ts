...
describe("initSessionState sender meta", () => {
  it("injects sender meta into BodyStripped for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-sender-meta-"));
  ...
  it("does not inject sender meta for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as AeonSageConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\baeonsage\\b", "(invalid"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("aeonsage")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    // Function removes zero-width characters and lowercases
    expect(normalizeMentionText("ae\u200bonsage")).toBe("aeonsage");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\baeonsage\\b"] } },
    });
    expect(matchesMentionPatterns("aeonsage: hi", regexes)).toBe(true);
  });
