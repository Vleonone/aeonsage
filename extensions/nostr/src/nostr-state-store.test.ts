...
import type { PluginRuntime } from "aeonsage/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.AEONSAGE_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-nostr-"));
  process.env.AEONSAGE_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.AEONSAGE_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".aeonsage");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.AEONSAGE_STATE_DIR;
    else process.env.AEONSAGE_STATE_DIR = previous;
