...
import type { AeonSageConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AeonSageConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: AeonSageConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AeonSageSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AeonSageConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AeonSageConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(
        applySessionDefaults(
          applySecurityModeDefaults(validated.data as AeonSageConfig),
        ),
      ),
    ),
  };
