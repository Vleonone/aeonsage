...
import { discoverAeonSagePlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { AeonSagePackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & ({ aeonsage?: AeonSagePackageManifest } | { [LEGACY_MANIFEST_KEY]?: AeonSagePackageManifest });
...
const ENV_CATALOG_PATHS = ["AEONSAGE_PLUGIN_CATALOG_PATHS", "AEONSAGE_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<AeonSagePackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: AeonSagePackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageAeonSage?: AeonSagePackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageAeonSage;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.aeonsage ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageAeonSage: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverAeonSagePlugins({ workspaceDir: options.workspaceDir });
