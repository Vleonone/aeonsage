...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.AEONSAGE_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.AEONSAGE_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.AEONSAGE_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.AEONSAGE_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.AEONSAGE_PROFILE,
      version: environment?.AEONSAGE_SERVICE_VERSION ?? env.AEONSAGE_SERVICE_VERSION,
    });
