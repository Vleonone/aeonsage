...
export const DEFAULT_LOG_DIR = "/tmp/aeonsage";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "aeonsage.log"); // legacy single-file path

const LOG_PREFIX = "aeonsage";
...
function resolveSettings(): ResolvedSettings {
  let cfg: AeonSageConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => AeonSageConfig;
      };
