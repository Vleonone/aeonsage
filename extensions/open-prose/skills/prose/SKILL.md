...
metadata: {"aeonsage":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == AeonSage `sessions_spawn`
- **File I/O** == AeonSage `read`/`write`
- **Remote fetch** == AeonSage `web_fetch` (or `exec` with curl when POST is required)
