...
export function resolveHooksConfig(cfg: AeonSageConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-aeonsage-token"] === "string"
      ? req.headers["x-aeonsage-token"].trim()
      : "";
  if (headerToken) return { token: headerToken, fromQuery: false };
  // SECURITY: reject query-parameter tokens — they leak via logs, referrer headers, and browser history
