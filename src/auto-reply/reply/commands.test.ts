...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-commands-"));
...
function buildParams(commandBody: string, cfg: AeonSageConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
