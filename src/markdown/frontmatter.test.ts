...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { aeonsage?: { emoji?: string } };
    expect(parsed.aeonsage?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { aeonsage?: { events?: string[] } };
