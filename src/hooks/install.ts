...
} & ({ aeonsage?: { hooks?: string[] } } | { [LEGACY_MANIFEST_KEY]?: { hooks?: string[] } });
...
async function ensureAeonSageHooks(manifest: HookPackageManifest) {
  const hooks = manifest.aeonsage?.hooks ?? manifest[LEGACY_MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing aeonsage.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json aeonsage.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureAeonSageHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-hook-"));
...
export async function installHooksFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-hook-pack-"));
