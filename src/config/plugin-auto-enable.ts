import type { AeonSageConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: AeonSageConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AeonSageConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AeonSageConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AeonSageConfig): boolean {
...
function isIMessageConfigured(cfg: AeonSageConfig): boolean {
...
function isWhatsAppConfigured(cfg: AeonSageConfig): boolean {
...
function isGenericChannelConfigured(cfg: AeonSageConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AeonSageConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AeonSageConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: AeonSageConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: AeonSageConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: AeonSageConfig, pluginId: string): AeonSageConfig {
...
function enablePluginEntry(cfg: AeonSageConfig, pluginId: string): AeonSageConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
...
export function applyPluginAutoEnable(params: {
  ...
  for (const entry of configured) {
    ...
    // Skip channel IDs - they are auto-enabled via channels config, not plugins.entries
    const isChannel = CHANNEL_PLUGIN_IDS.includes(entry.pluginId);
