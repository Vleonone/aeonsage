...
IMAGE_NAME="aeonsage-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/aeonsage-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/aeonsage-bin/systemctl

  cat > /tmp/aeonsage-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/aeonsage-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/aeonsage"
  npm_entry="/tmp/npm-prefix/lib/node_modules/aeonsage/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/aeonsage-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/aeonsage-gateway.service"
    if [ ! -f "$unit_path" ]; then
