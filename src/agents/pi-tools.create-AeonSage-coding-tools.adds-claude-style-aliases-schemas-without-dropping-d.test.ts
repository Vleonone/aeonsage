...
const defaultTools = createAeonSageCodingTools();

describe("createAeonSageCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createAeonSageCodingTools();
    ...
    try {
      ...
      const contents = "Hello from aeonsage read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "aeonsage-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "aeonsage-workspace"),
    ...
    const tools = createAeonSageCodingTools({ sandbox });
