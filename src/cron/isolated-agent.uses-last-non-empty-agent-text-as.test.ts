...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "aeonsage-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".aeonsage", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AeonSageConfig> = {},
): AeonSageConfig {
  const base: AeonSageConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "aeonsage"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".aeonsage", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
