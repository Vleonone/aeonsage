...
import { createAeonSageCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("aeonsage-ws-", async (workspaceDir) => {
      await withTempDir("aeonsage-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAeonSageCodingTools({ workspaceDir });
    ...
    await withTempDir("aeonsage-ws-", async (workspaceDir) => {
      await withTempDir("aeonsage-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello aeonsage");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("aeonsage-ws-", async (workspaceDir) => {
      await withTempDir("aeonsage-override-", async (overrideDir) => {
        const tools = createAeonSageCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("aeonsage-sandbox-", async (sandboxDir) => {
      await withTempDir("aeonsage-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "aeonsage-sbx-test",
        ...
        const tools = createAeonSageCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
