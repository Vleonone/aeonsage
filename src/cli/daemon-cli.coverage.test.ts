...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    AEONSAGE_STATE_DIR: process.env.AEONSAGE_STATE_DIR,
    AEONSAGE_CONFIG_PATH: process.env.AEONSAGE_CONFIG_PATH,
    AEONSAGE_GATEWAY_PORT: process.env.AEONSAGE_GATEWAY_PORT,
    AEONSAGE_PROFILE: process.env.AEONSAGE_PROFILE,
  };

  beforeEach(() => {
    process.env.AEONSAGE_STATE_DIR = "/tmp/aeonsage-cli-state";
    process.env.AEONSAGE_CONFIG_PATH = "/tmp/aeonsage-cli-state/aeonsage.json";
    delete process.env.AEONSAGE_GATEWAY_PORT;
    delete process.env.AEONSAGE_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.AEONSAGE_STATE_DIR !== undefined)
      process.env.AEONSAGE_STATE_DIR = originalEnv.AEONSAGE_STATE_DIR;
    else delete process.env.AEONSAGE_STATE_DIR;

    if (originalEnv.AEONSAGE_CONFIG_PATH !== undefined)
      process.env.AEONSAGE_CONFIG_PATH = originalEnv.AEONSAGE_CONFIG_PATH;
    else delete process.env.AEONSAGE_CONFIG_PATH;

    if (originalEnv.AEONSAGE_GATEWAY_PORT !== undefined)
      process.env.AEONSAGE_GATEWAY_PORT = originalEnv.AEONSAGE_GATEWAY_PORT;
    else delete process.env.AEONSAGE_GATEWAY_PORT;

    if (originalEnv.AEONSAGE_PROFILE !== undefined)
      process.env.AEONSAGE_PROFILE = originalEnv.AEONSAGE_PROFILE;
    else delete process.env.AEONSAGE_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AEONSAGE_PROFILE: "dev",
        AEONSAGE_STATE_DIR: "/tmp/aeonsage-daemon-state",
        AEONSAGE_CONFIG_PATH: "/tmp/aeonsage-daemon-state/aeonsage.json",
        AEONSAGE_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/com.aeonsage.gateway.plist",
    });
