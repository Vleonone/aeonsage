...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "AeonSage";
  if (/aeonsage/i.test(trimmed)) return trimmed;
  return `${trimmed} (AeonSage)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.AEONSAGE_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "aeonsage");
  ...
  const binCli = path.join(cwd, "bin", "aeonsage.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.AEONSAGE_TAILNET_DNS?.trim();
