...
// Section metadata with bilingual support
function getSectionMeta(lang: 'zh-CN' | 'en-US'): Record<string, { label: string; description: string }> {
  const isZh = lang === 'zh-CN';
  return {
    env: {
      label: isZh ? "环境变量" : "Environment Variables",
      description: isZh ? "传递给网关进程的环境变量" : "Environment variables passed to the gateway process"
    },
    update: {
      label: isZh ? "更新" : "Updates",
      description: isZh ? "自动更新设置和发布渠道" : "Auto-update settings and release channel"
    },
    agents: {
      label: isZh ? "智能体" : "Agents",
      description: isZh ? "智能体配置、模型和身份" : "Agent configurations, models, and identities"
    },
    auth: {
      label: isZh ? "认证" : "Authentication",
      description: isZh ? "API 密钥和认证配置" : "API keys and authentication profiles"
    },
    channels: {
      label: isZh ? "频道" : "Channels",
      description: isZh ? "消息频道 (Telegram, Discord, Slack 等)" : "Messaging channels (Telegram, Discord, Slack, etc.)"
    },
    messages: {
      label: isZh ? "消息" : "Messages",
      description: isZh ? "消息处理和路由设置" : "Message handling and routing settings"
    },
    commands: {
      label: isZh ? "命令" : "Commands",
      description: isZh ? "自定义斜杠命令" : "Custom slash commands"
    },
    hooks: {
      label: isZh ? "钩子" : "Hooks",
      description: isZh ? "Webhooks 和事件钩子" : "Webhooks and event hooks"
    },
    skills: {
      label: isZh ? "技能" : "Skills",
      description: isZh ? "技能包和能力扩展" : "Skill packs and capabilities"
    },
    tools: {
      label: isZh ? "工具" : "Tools",
      description: isZh ? "工具配置 (浏览器、搜索等)" : "Tool configurations (browser, search, etc.)"
    },
    gateway: {
      label: isZh ? "网关" : "Gateway",
      description: isZh ? "网关服务器设置 (端口、认证、绑定)" : "Gateway server settings (port, auth, binding)"
    },
    wizard: {
      label: isZh ? "设置向导" : "Setup Wizard",
      description: isZh ? "设置向导状态和历史记录" : "Setup wizard state and history"
    },
    meta: {
      label: isZh ? "元数据" : "Metadata",
      description: isZh ? "网关元数据和版本信息" : "Gateway metadata and version information"
    },
    logging: {
      label: isZh ? "日志" : "Logging",
      description: isZh ? "日志级别和输出配置" : "Log levels and output configuration"
    },
    browser: {
      label: isZh ? "浏览器" : "Browser",
      description: isZh ? "浏览器自动化设置" : "Browser automation settings"
    },
    ui: {
      label: isZh ? "界面" : "UI",
      description: isZh ? "用户界面偏好设置" : "User interface preferences"
    },
    models: {
      label: isZh ? "模型" : "Models",
      description: isZh ? "AI 模型配置和提供商" : "AI model configurations and providers"
    },
    bindings: {
      label: isZh ? "绑定" : "Bindings",
      description: isZh ? "按键绑定和快捷键" : "Key bindings and shortcuts"
    },
    broadcast: {
      label: isZh ? "广播" : "Broadcast",
      description: isZh ? "广播和通知设置" : "Broadcast and notification settings"
    },
    audio: {
      label: isZh ? "音频" : "Audio",
      description: isZh ? "音频输入/输出设置" : "Audio input/output settings"
    },
    session: {
      label: isZh ? "会话" : "Session",
      description: isZh ? "会话管理和持久化" : "Session management and persistence"
    },
    cron: {
      label: isZh ? "定时任务" : "Cron",
      description: isZh ? "计划任务和自动化" : "Scheduled tasks and automation"
    },
    web: {
      label: isZh ? "网页" : "Web",
      description: isZh ? "Web 服务器和 API 设置" : "Web server and API settings"
    },
    discovery: {
      label: isZh ? "发现" : "Discovery",
      description: isZh ? "服务发现和网络" : "Service discovery and networking"
    },
    canvasHost: {
      label: isZh ? "画布主机" : "Canvas Host",
      description: isZh ? "画布渲染和显示" : "Canvas rendering and display"
    },
    talk: {
      label: isZh ? "语音" : "Talk",
      description: isZh ? "语音和语音设置" : "Voice and speech settings"
    },
    plugins: {
      label: isZh ? "插件" : "Plugins",
      description: isZh ? "插件管理和扩展" : "Plugin management and extensions"
    },
    wallet: {
      label: isZh ? "钱包" : "Wallet",
      description: isZh ? "钱包 RPC 配置用于余额查询" : "Wallet RPC configuration for balance lookups"
    },
    diagnostics: {
      label: isZh ? "诊断" : "Diagnostics",
      description: isZh ? "系统诊断和调试信息" : "System diagnostics and debug info"
    },
    nodeHost: {
      label: isZh ? "节点主机" : "NodeHost",
      description: isZh ? "节点主机配置" : "Node host configuration"
    },
    media: {
      label: isZh ? "媒体" : "Media",
      description: isZh ? "媒体处理设置" : "Media processing settings"
    },
    approvals: {
      label: isZh ? "审批" : "Approvals",
      description: isZh ? "执行审批和权限" : "Execution approvals and permissions"
    },
  };
}

// Legacy export for compatibility
export const SECTION_META = getSectionMeta('en-US');
...
export function renderConfigForm(props: ConfigFormProps) {
  ...
  const sectionMeta = getSectionMeta(lang);

  if (!props.schema) {
    return html`<div class="muted">${lang === 'zh-CN' ? '架构不可用。' : 'Schema unavailable.'}</div>`;
  }
  const schema = props.schema;
  const value = props.value ?? {};
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`<div class="callout danger">${lang === 'zh-CN' ? '不支持的架构。请使用原始模式。' : 'Unsupported schema. Use Raw.'}</div>`;
  }
