...
export function listBlueBubblesAccountIds(cfg: AeonSageConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: AeonSageConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: AeonSageConfig): ResolvedBlueBubblesAccount[] {
