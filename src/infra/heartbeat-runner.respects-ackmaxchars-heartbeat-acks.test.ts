...
// NOTE: Delivery is intentionally skipped when channel is not ready.
// Tests must NOT assert send() in this state.
...
describe("runHeartbeatOnce", () => {
  it("respects ackMaxChars for heartbeat acks", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-hb-"));
    ...
    try {
      const cfg: AeonSageConfig = {
        ...
        channels: {
          whatsapp: {
            allowFrom: ["*"],
            heartbeat: { showOk: true }, // Enable OK token delivery so we can assert on a call
          },
        },
      ...
      // With ackMaxChars: 0, "HEARTBEAT_OK" is stripped to empty string,
      // which has length 0. 0 <= 0 is true, so the reply is skipped.
      // But because showOk: true is set, it should send the OK token.
