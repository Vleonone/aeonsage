...
function stripUnknownConfigKeys(config: AeonSageConfig): {
  config: AeonSageConfig;
  removed: string[];
} {
  const parsed = AeonSageSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as AeonSageConfig;
...
function noteOpencodeProviderOverrides(cfg: AeonSageConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: AeonSageConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as AeonSageConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("aeonsage doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("aeonsage doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "aeonsage doctor --fix" to remove these keys.');
    }
  }
