...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "AEONSAGE_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AEONSAGE_STATE_DIR = path.join(home, ".aeonsage");
    ...
    await fs.mkdir(path.join(home, ".aeonsage"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.aeonsage${path.sep}media`);
