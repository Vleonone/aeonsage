...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aeonsage-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aeonsage-ext-path-"));
    ...
    try {
      ...
      if (prev === undefined) delete process.env.AEONSAGE_STATE_DIR;
      else process.env.AEONSAGE_STATE_DIR = prev;
    }
  });
});
