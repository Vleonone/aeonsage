  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "aeonsage", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "aeonsage", input: "hi" },
        { "x-aeonsage-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "aeonsage:beta", input: "hi" });
