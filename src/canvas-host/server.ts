...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.aeonsageCanvasA2UIAction ||
        window.webkit.messageHandlers.aeonsageCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.aeonsageCanvasA2UIAction &&
        typeof window.aeonsageCanvasA2UIAction.postMessage === "function") ||
      (window.aeonsageCanvasA2UIAction &&
        typeof window.aeonsageCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.aeonsageSendUserAction === "function" ? window.aeonsageSendUserAction : undefined;
  if (!window.aeonsageSendUserAction && legacySend) {
    window.aeonsageSendUserAction = legacySend;
  }
  if (!window.aeonsageSendUserAction && typeof window.aeonsageSendUserAction === "function") {
    window.aeonsageSendUserAction = window.aeonsageSendUserAction;
  }
  const hasHelper = () =>
    typeof window.aeonsageSendUserAction === "function" ||
    typeof window.aeonsageSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("aeonsage:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AeonSage node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.aeonsageSendUserAction === "function"
        ? window.aeonsageSendUserAction
        : window.aeonsageSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AEONSAGE_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "aeonsage", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AeonSage Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
