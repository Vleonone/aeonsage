...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.aeonsage.org/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AeonSage hooks")
    ...
    .option("--hook-url <url>", "AeonSage hook URL")
    .option("--hook-token <token>", "AeonSage hook token")
