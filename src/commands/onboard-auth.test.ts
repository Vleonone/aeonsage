...
const requireAgentDir = () => {
  const agentDir = process.env.AEONSAGE_AGENT_DIR;
  if (!agentDir) throw new Error("AEONSAGE_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.AEONSAGE_STATE_DIR;
  const previousAgentDir = process.env.AEONSAGE_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.AEONSAGE_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.AEONSAGE_AGENT_DIR;
    } else {
      process.env.AEONSAGE_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.AEONSAGE_OAUTH_DIR;
  });

  it("writes auth-profiles.json under AEONSAGE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-oauth-"));
    process.env.AEONSAGE_STATE_DIR = tempStateDir;
    process.env.AEONSAGE_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AEONSAGE_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AEONSAGE_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-minimax-"));
    process.env.AEONSAGE_STATE_DIR = tempStateDir;
    process.env.AEONSAGE_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AEONSAGE_AGENT_DIR;
