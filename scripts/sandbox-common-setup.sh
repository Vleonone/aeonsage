...
BASE_IMAGE="${BASE_IMAGE:-aeonsage-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-aeonsage-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=aeonsage.sandbox=1)
NOTE
