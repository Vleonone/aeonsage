...
// NOTE(Vleonone): Upstream read now does file-magic MIME detection; we keep the wrapper
...
export function createSandboxedReadTool(root: string) {
  ...
  return wrapSandboxPathGuard(createAeonSageReadTool(base), root);
}
...
export function createAeonSageReadTool(base: AnyAgentTool): AnyAgentTool {
