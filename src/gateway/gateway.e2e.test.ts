...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.AEONSAGE_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "aeonsage");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.aeonsage-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".aeonsage");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "aeonsage.json");
      ...
      process.env.AEONSAGE_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.AEONSAGE_CONFIG_PATH = prev.configPath;
        process.env.AEONSAGE_GATEWAY_TOKEN = prev.token;
        process.env.AEONSAGE_SKIP_CHANNELS = prev.skipChannels;
        process.env.AEONSAGE_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.AEONSAGE_SKIP_CRON = prev.skipCron;
        process.env.AEONSAGE_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.AEONSAGE_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-wizard-home-"));
    process.env.HOME = tempHome;
    delete process.env.AEONSAGE_STATE_DIR;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.AEONSAGE_STATE_DIR = prev.stateDir;
      process.env.AEONSAGE_CONFIG_PATH = prev.configPath;
      process.env.AEONSAGE_GATEWAY_TOKEN = prev.token;
      process.env.AEONSAGE_SKIP_CHANNELS = prev.skipChannels;
      process.env.AEONSAGE_SKIP_GMAIL_WATCHER = prev.skipGmail;
