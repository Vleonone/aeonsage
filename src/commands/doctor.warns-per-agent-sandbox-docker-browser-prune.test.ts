...
describe("doctor command", () => {
  ...
  it("warns when extra workspace directories exist", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/Vleonone/aeonsage" } },
      },
      issues: [],
      legacyIssues: [],
    });
    ...
    note.mockClear();
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/Users/Vleonone");
    const realExists = fs.existsSync;
    const legacyPath = path.join("/Users/Vleonone", "aeonsage");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (
        value === "/Users/Vleonone/aeonsage" ||
        value === legacyPath ||
