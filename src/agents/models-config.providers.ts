...
type ModelsConfig = NonNullable<AeonSageConfig["models"]>;
...
const BAIDU_QIANFAN_BASE_URL = "https://qianfan.baidubce.com/v2";
const BAIDU_QIANFAN_DEFAULT_CONTEXT_WINDOW = 128000;
const BAIDU_QIANFAN_DEFAULT_MAX_TOKENS = 8192;
const BAIDU_QIANFAN_DEFAULT_COST = {
  input: 0,
  output: 0,
  cacheRead: 0,
  cacheWrite: 0,
...
function buildBaiduQianfanProvider(): ProviderConfig {
  return {
    ...
    models: [
      {
        ...
        reasoning: false,
        input: ["text"],
        contextWindow: BAIDU_QIANFAN_DEFAULT_CONTEXT_WINDOW,
        maxTokens: BAIDU_QIANFAN_DEFAULT_MAX_TOKENS,
        cost: BAIDU_QIANFAN_DEFAULT_COST,
      },
    ],
  };
}

function buildCloudflareAiGatewayProvider(params: {
  accountId: string;
  gatewayId: string;
}): ProviderConfig {
  const baseUrl = `${CLOUDFLARE_AI_GATEWAY_BASE_URL}/${params.accountId}/${params.gatewayId}/openai`;
  return {
    baseUrl,
    api: "openai-completions",
    models: [
      {
        id: "claude-opus-4-6",
        name: "Claude Opus 4.6 (CF Gateway)",
        reasoning: true,
        input: ["text", "image"],
        cost: CLOUDFLARE_DEFAULT_COST,
        contextWindow: 200000,
        maxTokens: 32768,
      },
      {
        id: "gpt-5.3-codex",
        name: "GPT 5.3 Codex (CF Gateway)",
        reasoning: true,
        input: ["text"],
        cost: CLOUDFLARE_DEFAULT_COST,
        contextWindow: 200000,
...
export async function resolveImplicitProviders(params: {
  ...
  const baiduKey =
    resolveEnvApiKeyVarName("baidu-qianfan") ??
    resolveApiKeyFromProfiles({ provider: "baidu-qianfan", store: authStore });
  if (baiduKey) {
    providers["baidu-qianfan"] = { ...buildBaiduQianfanProvider(), apiKey: baiduKey };
  }

  // Cloudflare AI Gateway - requires account ID and gateway ID
  const cfAccountId = process.env.CLOUDFLARE_ACCOUNT_ID?.trim();
  const cfGatewayId = process.env.CLOUDFLARE_AI_GATEWAY_ID?.trim();
  const cfApiKey =
    resolveEnvApiKeyVarName("cloudflare") ??
    resolveApiKeyFromProfiles({ provider: "cloudflare", store: authStore }) ??
    process.env.CLOUDFLARE_API_KEY?.trim();
  if (cfAccountId && cfGatewayId && cfApiKey) {
    providers.cloudflare = {
      ...buildCloudflareAiGatewayProvider({
        accountId: cfAccountId,
        gatewayId: cfGatewayId,
      }),
      apiKey: typeof cfApiKey === "string" ? cfApiKey : cfApiKey,
    };
  }
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in AeonSage's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within AeonSage's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // AeonSage uses its own auth store and exchanges tokens at runtime.
  // `models list` uses AeonSage's auth heuristics for availability.
