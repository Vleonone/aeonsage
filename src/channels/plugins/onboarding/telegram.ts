...
function setTelegramDmPolicy(cfg: AeonSageConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("aeonsage logs --follow")}\` (safest)`,
...
export const telegramOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  getStatus: async ({ cfg }) => {
    ...
    return {
      ...
      selectionHint: configured ? "recommended 路 configured" : "recommended 路 newcomer-friendly",
