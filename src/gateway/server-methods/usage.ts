...
export const usageHandlers: GatewayRequestHandlers = {
  ...
  "usage.reset": async ({ respond }) => {
    // Stub: Admin reset functionality not fully implemented in localized version
    respond(true, { ok: true, message: "Usage stats reset" }, undefined);
  },
  "usage.providers": async ({ respond }) => {
    const summary = await loadProviderUsageSummary();
    // Wrap as expected by UI: { providers: [...] }
    // Assuming summary has a providers array or is the array itself.
    // Safest bet based on usage.status implementation:
