...
/**
 * Cleanup suspended AeonSage CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AeonSageConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-cli-images-"));
  const paths: string[] = [];
