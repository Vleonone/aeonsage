...
const execHostEnforced = process.env.AEONSAGE_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.AEONSAGE_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureAeonSageCliOnPath({ pathEnv: process.env.PATH ?? "" });
