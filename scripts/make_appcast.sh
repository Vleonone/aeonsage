...
ZIP=${1:?"Usage: $0 AeonSage-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/aeonsage/aeonsage/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^AeonSage-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/aeonsage/aeonsage/releases/download/v${VERSION}/"}
