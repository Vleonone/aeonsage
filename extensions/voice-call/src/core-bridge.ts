...
function resolveAeonSageRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.AEONSAGE_ROOT?.trim() || process.env.AEONSAGE_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["aeonsage", "aeonsage"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set AEONSAGE_ROOT (or legacy AEONSAGE_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveAeonSageRoot();
