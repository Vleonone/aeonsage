...
metadata: {"aeonsage":{"emoji":"🐦","requires":{"bins":["bird"]},"install":[{"id":"brew","kind":"brew","formula":"Vleonone/tap/bird","bins":["bird"],"label":"Install bird (brew)","os":["darwin"]},{"id":"npm","kind":"node","package":"@Vleonone/bird","bins":["bird"],"label":"Install bird (npm)"}]}}
---
...
```bash
# npm/pnpm/bun
npm install -g @Vleonone/bird

# Homebrew (macOS, prebuilt binary)
brew install Vleonone/tap/bird

# One-shot (no install)
bunx @Vleonone/bird whoami
```
...
bird search "from:Vleonone" --all --max-pages 3
```
