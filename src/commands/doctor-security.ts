...
import type { AeonSageConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AeonSageConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("aeonsage security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("aeonsage configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("aeonsage config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("aeonsage doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("aeonsage config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
