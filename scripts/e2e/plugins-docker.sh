...
IMAGE_NAME="aeonsage-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/aeonsage-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.aeonsage/extensions"

  cat > "$HOME/.aeonsage/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/aeonsage-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@aeonsage/demo-plugin-tgz",
  "version": "0.0.1",
  "aeonsage": { "extensions": ["./index.js"] }
}
JSON
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/aeonsage-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@aeonsage/demo-plugin-dir",
  "version": "0.0.1",
  "aeonsage": { "extensions": ["./index.js"] }
}
JSON
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/aeonsage-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@aeonsage/demo-plugin-file",
  "version": "0.0.1",
  "aeonsage": { "extensions": ["./index.js"] }
}
JSON
