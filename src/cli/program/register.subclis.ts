...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.AEONSAGE_DISABLE_LAZY_SUBCOMMANDS)) return false;
  if (hasHelpOrVersion(argv)) return false;
  return true;
};

const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.AEONSAGE_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<AeonSageConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "kali",
    description: "⚠️ Kali security assessment tools",
    register: async (program) => {
      const mod = await import("./register.kali.js");
      mod.registerKaliCommands(program);
    },
  },
  {
    name: "skills",
