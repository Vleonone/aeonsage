...
const SEARCH_TOOL = "https://docs.aeonsage.org/mcp.SearchAeonSage";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.aeonsage.org");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('aeonsage docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.aeonsage.org/");
      runtime.log(`Search: ${formatCliCommand('aeonsage docs "your query"')}`);
    }
    return;
  }
