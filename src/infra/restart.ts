...
export function triggerAeonSageRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.AEONSAGE_SYSTEMD_UNIT,
        process.env.AEONSAGE_PROFILE,
      );
  ...
  const label =
    process.env.AEONSAGE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AEONSAGE_PROFILE);
