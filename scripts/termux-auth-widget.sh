...
# AeonSage Auth Widget for Termux
...
SERVER="${AEONSAGE_SERVER:-l36}"

# Check auth status
termux-toast "Checking AeonSage auth..."

STATUS=$(ssh "$SERVER" '$HOME/aeonsage/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/aeonsage/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AEONSAGE_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/aeonsage/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|AEONSAGE_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AeonSage won't work until re-auth"
                ;;
        esac
        ;;
