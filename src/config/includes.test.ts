...
const ETC_AEONSAGE_DIR = path.join(ROOT_DIR, "etc", "aeonsage");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "aeonsage.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAeonSagePath(...parts: string[]) {
  return path.join(ETC_AEONSAGE_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAeonSagePath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "aeonsage.json"))).toEqual({
      shared: true,
    });
  });
});
