...
import { resolveAeonSagePackageRoot } from "../infra/aeonsage-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "aeonsage doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "aeonsage";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "aeonsage"]);
const CLI_NAME = resolveCliName();
const AEONSAGE_REPO_URL = "https://github.com/aeonsage/aeonsage.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "aeonsage");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("aeonsage@")) return trimmed.slice("aeonsage@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.AEONSAGE_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AEONSAGE_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AEONSAGE_GIT_DIR points at a non-git directory: ${params.dir}. Set AEONSAGE_GIT_DIR to an empty folder or a aeonsage checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AEONSAGE_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveAeonSagePackageRoot({
  ...
  const rows = [
    ...
    {
      Item: "Update",
      Value: updateAvailability.available ? theme.warn(`available 路 ${updateLine}`) : updateLine,
    },
  ];

  defaultRuntime.log(theme.heading("AeonSage update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating AeonSage..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this AeonSage install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("aeonsage doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("aeonsage gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g aeonsage@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g aeonsage@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.AEONSAGE_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.AEONSAGE_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("aeonsage gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aeonsage doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("aeonsage gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aeonsage gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `aeonsage update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `AEONSAGE_GIT_DIR points at a non-git directory: ${gitDir}. Set AEONSAGE_GIT_DIR to an empty folder or a aeonsage checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AEONSAGE_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AeonSage to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["aeonsage update", "Update a source checkout (git)"],
        ["aeonsage update --channel beta", "Switch to beta channel (git + npm)"],
        ["aeonsage update --channel dev", "Switch to dev channel (git + npm)"],
        ["aeonsage update --tag beta", "One-off update to a dist-tag or version"],
        ["aeonsage update --no-restart", "Update without restarting the service"],
        ["aeonsage update --json", "Output result as JSON"],
        ["aeonsage update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aeonsage update wizard", "Interactive update wizard"],
        ["aeonsage --update", "Shorthand for aeonsage update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aeonsage update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.aeonsage.org/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aeonsage update status", "Show channel + version status."],
          ["aeonsage update status --json", "JSON output."],
          ["aeonsage update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.aeonsage.org/cli/update")}`,
    )
    .action(async (opts) => {
      try {
