...
import { resolveAeonSageAgentDir } from "./agent-paths.js";
import {
...
import { ensureAeonSageModelsJson } from "./models-config.js";

const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.AEONSAGE_LIVE_TEST);
const SETUP_TOKEN_RAW = process.env.AEONSAGE_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.AEONSAGE_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.AEONSAGE_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const SETUP_TOKEN_MODEL = process.env.AEONSAGE_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "AEONSAGE_LIVE_SETUP_TOKEN did not look like a setup-token. Use AEONSAGE_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set AEONSAGE_LIVE_SETUP_TOKEN_VALUE or AEONSAGE_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureAeonSageModelsJson(cfg, tokenSource.agentDir);
