...
function candidateBinDirs(opts: EnsureAeonSagePathOpts): string[] {
  ...
  // Bundled macOS app: `aeonsage` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingAeonSage = path.join(execDir, "aeonsage");
    if (isExecutable(siblingAeonSage)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/aeonsage` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "aeonsage"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `aeonsage` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAeonSageCliOnPath(opts: EnsureAeonSagePathOpts = {}) {
  if (isTruthyEnvValue(process.env.AEONSAGE_PATH_BOOTSTRAPPED)) return;
  process.env.AEONSAGE_PATH_BOOTSTRAPPED = "1";
