...
type AgentConfig = Partial<NonNullable<NonNullable<AeonSageConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.aeonsage/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🌌 AeonSage ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: AeonSageConfig): string {
  const lines = ["ℹ️ Help", ""];
