...
import { readConfigFileSnapshot, type AeonSageConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: AeonSageConfig, model: string): AeonSageConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("aeonsage plugins install")}\`.`,
    );
  }
