...
# Build and bundle AeonSage into a minimal .app we can open.
# Outputs to dist/AeonSage.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AeonSage.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AeonSage"
BUNDLE_ID="${BUNDLE_ID:-com.aeonsage.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/aeonsage/aeonsage/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AeonSage/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AeonSageBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AeonSageGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AeonSage"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AeonSage"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AeonSage"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AeonSage" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AeonSage/Resources/AeonSage.icns" "$APP_ROOT/Contents/Resources/AeonSage.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AeonSage/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AeonSageKit resources"
AEONSAGEKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AeonSageKit_AeonSageKit.bundle"
if [ -d "$AEONSAGEKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AeonSageKit_AeonSageKit.bundle"
  cp -R "$AEONSAGEKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AeonSageKit_AeonSageKit.bundle"
else
  echo "WARN: AeonSageKit resource bundle not found at $AEONSAGEKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AeonSage"
killall -q AeonSage 2>/dev/null || true
