...
import { resolveAeonSageAgentDir } from "../agents/agent-paths.js";
...
import { ensureAeonSageModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { AeonSageConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.AEONSAGE_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AEONSAGE_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.AEONSAGE_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AeonSageConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.AEONSAGE_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.AEONSAGE_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveAeonSageAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-live-state-"));
  process.env.AEONSAGE_STATE_DIR = tempStateDir;
  ...
  process.env.AEONSAGE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.aeonsage-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAeonSageAgentDir();
  const sanitizedCfg: AeonSageConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "AeonSage live tool probe (local, safe): " +
    ...
    process.env.AEONSAGE_CONFIG_PATH = previous.configPath;
    process.env.AEONSAGE_GATEWAY_TOKEN = previous.token;
    process.env.AEONSAGE_SKIP_CHANNELS = previous.skipChannels;
    process.env.AEONSAGE_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.AEONSAGE_SKIP_CRON = previous.skipCron;
    process.env.AEONSAGE_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.AEONSAGE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.AEONSAGE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAeonSageModelsJson(cfg);
      ...
      const rawModels = process.env.AEONSAGE_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.aeonsage-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
