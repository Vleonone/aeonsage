...
// NOTE: This extension is intended to be bundled with AeonSage.
// When running from source (tests/dev), AeonSage internals live under src/.
...
export function createLlmTaskTool(api: AeonSagePluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via aeonsage.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-llm-task-"));
