...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `aeonsage-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.AEONSAGE_CONFIG_PATH = path.join(root, "aeonsage.json");
};
...
vi.mock("@mariozechner/pi-coding-agent", async () => {
  ...
  // Mock ModelRegistry to intercept model discovery.
  const OrigModelRegistry = actual.ModelRegistry;
  class MockModelRegistry extends OrigModelRegistry {
    constructor(...args: ConstructorParameters<typeof OrigModelRegistry>) {
      super(...args);
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "aeonsage.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "aeonsage-gateway-test"),
        ...fileDefaults,
