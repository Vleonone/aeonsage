const DEFAULT_TAGLINE = "All your chats, one AeonSage.";
...
const HOLIDAY_TAGLINES = {
  ...
  christmas:
    "Christmas: Ho ho ho—Aeon's little assistant is here to ship joy, roll back chaos, and stash the keys safely.",
...
const TAGLINES: string[] = [
  "Your terminal just expanded its horizon—type something and let the core handle the busywork.",
  ...
  "Automation with precision: minimal fuss, maximal intelligence.",
  ...
  "AeonSage in your contacts, but somehow less clingy than your group chats. 🌌",
  "I'll keep your workflow flowing through the cosmos, not boiling in a pot.",
  "Shell yeah—I'm here to streamline the toil and leave you the glory.",
  ...
  "Because you deserve a cosmic guide, not just another contact in your list.",
...
// ═══════════════════════════════════════════════════════════════
// Chinese Taglines (for zh locale)
// ═══════════════════════════════════════════════════════════════

const DEFAULT_TAGLINE_ZH = "所有聊天，一个 AeonSage。";

const _HOLIDAY_TAGLINES_ZH = {
  newYear: "元旦快乐：新年新配置——端口冲突依旧，但这次我们像成年人一样解决。",
  lunarNewYear: "新年快乐：祝你构建顺利、分支兴旺，合并冲突都被鞭炮赶跑！🧧",
  christmas: "圣诞快乐：Ho ho ho——AeonSage 小助手来送礼、回滚混乱、安全保管密钥。🎄",
  eid: "开斋节快乐：庆祝模式——队列已清空，任务已完成，好心情已提交到主分支。",
  diwali: "排灯节快乐：让日志闪耀，让 bug 逃跑——今天我们点亮终端，骄傲发布！🪔",
  easter: "复活节快乐：找到你丢失的环境变量了——就当是一次小型 CLI 寻蛋游戏。🐰",
  hanukkah: "光明节快乐：八夜八次重试，零羞耻——愿你的网关常亮，部署平安。🕎",
  halloween: "万圣节快乐：当心被诅咒的依赖、闹鬼的缓存、和 node_modules 的幽灵。🎃",
  thanksgiving: "感恩节快乐：感谢稳定的端口、正常的 DNS、和一个会读日志的机器人。🦃",
  valentines: "情人节快乐：玫瑰是类型化的，紫罗兰是管道化的——我自动化杂务，你陪伴人类。💕",
} as const;

const TAGLINES_ZH: string[] = [
  // Fallback to English to satisfy "No Chinese" requirement
  ...TAGLINES,
];
...
export function activeTaglinesZh(_options: TaglineOptions = {}): string[] {
  // Chinese taglines don't have holiday filtering yet
  return TAGLINES_ZH.length > 0 ? TAGLINES_ZH : [DEFAULT_TAGLINE_ZH];
}

/**
 * Pick a tagline. Supports locale detection via environment or explicit option.
 */
export function pickTagline(options: TaglineOptions = {}): string {
  const env = options.env ?? process.env;

  // Detect locale from environment or option
  const locale = options.locale ?? detectLocale(env);

  const override = env?.AEONSAGE_TAGLINE_INDEX;
  if (override !== undefined) {
    const parsed = Number.parseInt(override, 10);
    if (!Number.isNaN(parsed) && parsed >= 0) {
      const pool =
        locale === "zh"
          ? TAGLINES_ZH.length > 0
            ? TAGLINES_ZH
            : [DEFAULT_TAGLINE_ZH]
          : TAGLINES.length > 0
            ? TAGLINES
  ...
  const pool = locale === "zh" ? activeTaglinesZh(options) : activeTaglines(options);
 ...
 */
function detectLocale(env?: NodeJS.ProcessEnv): "en" | "zh" {
  const e = env ?? process.env;

  // AEONSAGE_LOCALE has highest priority - check for explicit 'en' first
  const explicitLocale = e?.AEONSAGE_LOCALE?.toLowerCase();
...
export { TAGLINES, TAGLINES_ZH, HOLIDAY_RULES, DEFAULT_TAGLINE, DEFAULT_TAGLINE_ZH, detectLocale };
