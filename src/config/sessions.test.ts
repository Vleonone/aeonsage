...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-aeonsage",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-aeonsage#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-sessions-"));
  ...
  it("derives session transcripts dir from AEONSAGE_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AEONSAGE_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.AEONSAGE_STATE_DIR;
    process.env.AEONSAGE_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.AEONSAGE_STATE_DIR = prev;
      }
    }
  });
