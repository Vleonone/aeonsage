...
function appendAeonSageHubHint(output: string, json?: boolean): string {
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("aeonsage skills list")}\` to see all skills.`
      : "No skills found.";
    return appendAeonSageHubHint(message, opts.json);
  }
  ...
  return appendAeonSageHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendAeonSageHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("aeonsage skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.aeonsage.org/cli/skills")}\n`,
    );
