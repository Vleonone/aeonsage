...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.AEONSAGE_AGENT_DIR;
    else process.env.AEONSAGE_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-chutes-"));
    process.env.AEONSAGE_STATE_DIR = tempDir;
    process.env.AEONSAGE_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
