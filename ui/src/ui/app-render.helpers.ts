...
import { type Language, setCurrentLanguage } from "./i18n";

export function renderTab(state: AppViewState, tab: Tab) {
  const href = pathForTab(tab, state.basePath);
  const title = titleForTab(tab, state.language);
  return html`
    <a
      href=${href}
...
export function renderLanguageToggle(state: AppViewState) {
  const toggleDropdown = () => {
    (state as any)._langDropdownOpen = !(state as any)._langDropdownOpen;
  ...
  const applyLanguage = (next: Language) => () => {
    ...
    (state as any)._langDropdownOpen = false;
    state.applySettings({ ...state.settings });
  };

  const isOpen = (state as any)._langDropdownOpen;

  return html`
    <div class="language-dropdown">
      <button 
        class="btn btn--sm btn--icon language-dropdown__trigger ${isOpen ? 'active' : ''}"
      ...
      ${isOpen ? html`
        <div class="dropdown-menu language-dropdown__menu">
          <button class="dropdown-item ${state.language === 'zh-CN' ? 'active' : ''}" @click=${applyLanguage('zh-CN')}>
            <span class="dropdown-item__text">中文 (ZH)</span>
            ${state.language === 'zh-CN' ? html`<span class="dropdown-item__check">${icons.check}</span>` : ''}
          </button>
          <button class="dropdown-item ${state.language === 'en-US' ? 'active' : ''}" @click=${applyLanguage('en-US')}>
            <span class="dropdown-item__text">English (EN)</span>
            ${state.language === 'en-US' ? html`<span class="dropdown-item__check">${icons.check}</span>` : ''}
          </button>
        </div>
      ` : ''}
    </div>
