...
import { scheduleGatewaySigusr1Restart, triggerAeonSageRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting aeonsage in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAeonSageRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting aeonsage via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
