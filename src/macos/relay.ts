...
declare const __AEONSAGE_VERSION__: string | undefined;
...
async function main() {
  ...
  const { ensureAeonSageCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[aeonsage] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
