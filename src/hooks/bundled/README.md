...
This directory contains hooks that ship with AeonSage. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/aeonsage`)
...
**Docs**: https://docs.aeonsage.org/hooks/soul-evil
...
metadata:
  { "aeonsage": { "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **emoji**: Display emoji for CLI (Removed for standardization)
...
Hooks can be configured in `~/.aeonsage/aeonsage.json`:
...
2. Restart gateway: `pkill -9 -f 'aeonsage.*gateway' && pnpm aeonsage gateway`
3. Enable the hook: `aeonsage hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.aeonsage.org/hooks
