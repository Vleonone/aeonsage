...
import { printCompactHeader } from "../terminal/header.js";
...
export async function statusCommand(
  ...
  const memoryValue = (() => {
    ...
    if (!memory) {
      ...
      return muted(`enabled (${slot}) 路 unavailable`);
    }
    const parts: string[] = [];
  ...
  const overviewRows = [
    ...
    { Item: "OS", Value: `${osSummary.label} 路 node ${process.versions.node}` },
    {
      Item: "Tailscale",
      Value:
        tailscaleMode === "off"
  ...
  printCompactHeader((msg) => runtime.log(msg));
  ...
  if (importantFindings.length === 0) {
    ...
    if (sorted.length > shown.length) {
      runtime.log(theme.muted(`鈥?+${sorted.length - shown.length} more`));
    }
  }
  runtime.log(theme.muted(`Full report: ${formatCliCommand("aeonsage security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("aeonsage security audit --deep")}`));
  ...
  if (summary.queuedSystemEvents.length > 0) {
    ...
    if (summary.queuedSystemEvents.length > 5) {
      runtime.log(muted(`鈥?+${summary.queuedSystemEvents.length - 5} more`));
    }
  }
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.aeonsage.org/faq");
  runtime.log("Troubleshooting: https://docs.aeonsage.org/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("aeonsage status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("aeonsage logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("aeonsage status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("aeonsage gateway probe")}`);
  }
}
