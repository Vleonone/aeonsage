// NOTE: Windows may experience EBUSY file locking errors during cleanup.
// Retry logic is implemented in afterAll to handle this platform-specific behavior.
...
describe("media server", () => {
  ...
  afterAll(async () => {
    // On Windows, files may still be locked briefly after server close
    // Retry cleanup a few times with delays
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        await fs.rm(MEDIA_DIR, { recursive: true, force: true });
        break;
      } catch (err) {
        if (attempt < 2 && (err as NodeJS.ErrnoException).code === "EBUSY") {
          await new Promise((r) => setTimeout(r, 100 * (attempt + 1)));
          continue;
        }
        // Ignore errors on final attempt - directory may already be gone
      }
    }
  });
  ...
  // Symlinks on Windows require admin privileges, skip this test
