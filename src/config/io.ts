...
import type { AeonSageConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareAeonSageVersions } from "./version.js";
...
function coerceConfig(value: unknown): AeonSageConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AeonSageConfig;
}
...
function stampConfigVersion(cfg: AeonSageConfig): AeonSageConfig {
...
function warnIfConfigFromFuture(cfg: AeonSageConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareAeonSageVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AeonSage (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: AeonSageConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AeonSageConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AeonSageConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AeonSageConfig, {
        env: deps.env,
      ...
      const cfg = applyModelDefaults(
        applyCompactionDefaults(
          applyContextPruningDefaults(
            applyAgentDefaults(
              applySessionDefaults(
                applySecurityModeDefaults(applyLoggingDefaults(applyMessageDefaults(validated.config))),
              ),
            ),
          ),
        ),
      ...
      const config = applyTalkApiKey(
        applyModelDefaults(
          applyCompactionDefaults(
            applyContextPruningDefaults(
              applyAgentDefaults(
                applySessionDefaults(applySecurityModeDefaults(applyMessageDefaults({}))),
              ),
            ),
          ),
        ),
  ...
  async function writeConfigFile(cfg: AeonSageConfig) {
    clearConfigCache();
...
// module scope. `AEONSAGE_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AEONSAGE_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AEONSAGE_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): AeonSageConfig {
...
export async function writeConfigFile(cfg: AeonSageConfig): Promise<void> {
  clearConfigCache();
