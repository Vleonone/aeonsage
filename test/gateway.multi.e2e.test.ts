...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `aeonsage-e2e-${name}-`));
  const configDir = path.join(homeDir, ".aeonsage");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          AEONSAGE_CONFIG_PATH: configPath,
          AEONSAGE_STATE_DIR: stateDir,
          AEONSAGE_GATEWAY_TOKEN: "",
          AEONSAGE_GATEWAY_PASSWORD: "",
          AEONSAGE_SKIP_CHANNELS: "1",
          AEONSAGE_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        AEONSAGE_GATEWAY_TOKEN: inst.gatewayToken,
        AEONSAGE_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AEONSAGE_GATEWAY_PORT: String(gwA.port),
          AEONSAGE_GATEWAY_TOKEN: gwA.gatewayToken,
          AEONSAGE_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AEONSAGE_GATEWAY_PORT: String(gwB.port),
          AEONSAGE_GATEWAY_TOKEN: gwB.gatewayToken,
          AEONSAGE_GATEWAY_PASSWORD: "",
        }),
