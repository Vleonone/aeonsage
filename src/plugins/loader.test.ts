...
import { loadAeonSagePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.AEONSAGE_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `aeonsage-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "aeonsage.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.AEONSAGE_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.AEONSAGE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadAeonSagePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AEONSAGE_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAeonSagePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAeonSagePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.AEONSAGE_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
