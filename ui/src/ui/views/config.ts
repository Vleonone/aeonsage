...
const sidebarIcons = {
  ...
  wallet: html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 7h18a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2z"></path><path d="M16 12h4"></path><circle cx="18" cy="12" r="1"></circle><path d="M3 7V5a2 2 0 0 1 2-2h12"></path></svg>`,
...
// Section definitions - now a function to support i18n
function getSections(texts: ReturnType<typeof t>) {
  return [
    { key: "env", label: texts.config.sections.environment },
    { key: "update", label: texts.config.sections.updates },
    { key: "agents", label: texts.config.sections.agents },
    { key: "auth", label: texts.config.sections.authentication },
    { key: "channels", label: texts.config.sections.channels },
    { key: "messages", label: texts.config.sections.messages },
    { key: "commands", label: texts.config.sections.commands },
    { key: "hooks", label: texts.config.sections.hooks },
    { key: "skills", label: texts.config.sections.skills },
    { key: "tools", label: texts.config.sections.tools },
    { key: "gateway", label: texts.config.sections.gateway },
    { key: "wallet", label: texts.config.sections.wallet },
    { key: "wizard", label: texts.config.sections.setupWizard },
  ];
}
...
export function renderConfig(props: ConfigProps) {
  ...
  const SECTIONS = getSections(texts);
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">${texts.nav.config}</div>
        ...
        <nav class="config-nav">
          ...
          >
            ...
            <span class="config-nav__label">${texts.nav.config}</span>
          </button>
        ...
        <div class="config-sidebar__footer">
          <div class="config-mode-toggle">
            ...
            >
              ${texts.config.formMode}
            </button>
            <button
              class="config-mode-toggle__btn ${props.formMode === "raw" ? "active" : ""}"
              @click=${() => props.onFormModeChange("raw")}
            >
              ${texts.config.rawMode}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${hasChanges ? html`
              <span class="config-changes-badge">${props.formMode === "raw" ? texts.config.unsavedChanges : `${diff.length} ${texts.config.unsavedChanges}`}</span>
            ` : html`
              <span class="config-status muted">${texts.config.noChanges}</span>
            `}
          </div>
          <div class="config-actions__right">
            <button class="btn btn--sm" ?disabled=${props.loading} @click=${props.onReload}>
              ${props.loading ? texts.common.loading : texts.config.reload}
            </button>
            <button
              ...
              @click=${props.onSave}
            >
              ${props.saving ? texts.common.loading : texts.config.save}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canApply}
              @click=${props.onApply}
            >
              ${props.applying ? texts.common.loading : texts.config.apply}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canUpdate}
            ...
            >
              ${props.updating ? texts.common.loading : texts.config.update}
            </button>
          </div>
        </div>
