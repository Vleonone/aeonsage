...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-embedded-agent-"));
...
const ensureModels = (cfg: AeonSageConfig) => ensureAeonSageModelsJson(cfg, agentDir) as unknown;
