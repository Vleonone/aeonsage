...
beforeEach(() => {
  ...
  resolveAeonSagePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AEONSAGE_STATE_DIR;
  originalUpdateInProgress = process.env.AEONSAGE_UPDATE_IN_PROGRESS;
  process.env.AEONSAGE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aeonsage-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AEONSAGE_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AEONSAGE_UPDATE_IN_PROGRESS;
  } else {
    process.env.AEONSAGE_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveAeonSagePackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadAeonSagePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/aeonsage-root.js", () => ({
  resolveAeonSagePackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "aeonsage-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AEONSAGE_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AEONSAGE_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/aeonsage.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AEONSAGE_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
