...
import { discoverAeonSagePlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AEONSAGE_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.AEONSAGE_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
