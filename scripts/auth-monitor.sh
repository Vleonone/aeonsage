...
#
# Suggested cron: */30 * * * * /home/admin/aeonsage/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AeonSage notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., aeonsage-alerts)
...
STATE_FILE="$HOME/.aeonsage/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AeonSage if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use aeonsage
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AeonSage to $NOTIFY_PHONE..."
            aeonsage send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AeonSage is down. Run: ssh l36 '~/aeonsage/scripts/mobile-reauth.sh'" "urgent"
    exit 1
