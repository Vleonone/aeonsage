...
function resolveDefaultAccountId(cfg: AeonSageConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: AeonSageConfig): boolean {
...
export function summarizeBindings(cfg: AeonSageConfig, bindings: AgentBinding[]): string[] {
