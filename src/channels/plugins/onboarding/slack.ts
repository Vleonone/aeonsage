...
function setSlackDmPolicy(cfg: AeonSageConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "AeonSage";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for AeonSage`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: AeonSageConfig, allowFrom: string[]): AeonSageConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
