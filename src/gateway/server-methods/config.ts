...
import { loadAeonSagePlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
function redactCredentials(obj: unknown): unknown {
  ...
  for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
    if (typeof value === "string" && CREDENTIAL_KEY_PATTERNS.some((p) => p.test(key))) {
      result[key] = value.length > 8 ? `${value.slice(0, 4)}...****` : "****";
...
export const configHandlers: GatewayRequestHandlers = {
  "config.get": async ({ params, respond }) => {
    ...
    respond(true, redactCredentials(snapshot), undefined);
  },
  "config.schema": ({ params, respond }) => {
    ...
    const pluginRegistry = loadAeonSagePlugins({
      config: cfg,
      workspaceDir,
      logger: {
        info: () => {},
