  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${texts.logs.title}</div>
          <div class="card-sub">${texts.logs.subtitle}</div>
        </div>
        <div class="row" style="gap: 8px;">
          <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
            ${props.loading ? texts.common.loading : texts.logs.refresh}
          </button>
          <button
            class="btn"
      ...
      props.onFilterTextChange((e.target as HTMLInputElement).value)}
        ...
        <label class="field checkbox">
          <span>${texts.logs.autoFollow}</span>
          <input
            type="checkbox"
      ...
      : repeat(
        filtered,
        (entry) => entry.time + entry.raw, // Key function
        (entry) => html`
            <div class="log-row">
