...
type TelegramVideoNoteOpts = {
  ...
  duration?: number;
  /** Video width and height (diameter of the circular video). */
  length?: number;
  /** Message ID to reply to (for threading) */
  replyToMessageId?: number;
  /** Forum topic thread ID (for forum supergroups) */
...
/**
 * Send a video note (circular video message) to a Telegram chat.
 * @param to - Chat ID or username
 * @param videoSource - URL or Buffer of the video
 */
export async function sendVideoNoteTelegram(
  ...
  if (typeof videoSource === "string") {
    const media = await loadWebMedia(videoSource);
    file = new InputFile(media.buffer, media.fileName ?? "video_note.mp4");
  } else {
    file = new InputFile(videoSource, "video_note.mp4");
  }

  const videoNoteParams: Record<string, unknown> = {
    ...(hasThreadParams ? threadParams : {}),
    ...(opts.duration != null ? { duration: Math.trunc(opts.duration) } : {}),
    ...(opts.length != null ? { length: Math.trunc(opts.length) } : {}),
  };
  ...
  const result = await requestWithDiag(
    () =>
      api.sendVideoNote(
        chatId,
        file,
        Object.keys(videoNoteParams).length > 0 ? videoNoteParams : undefined,
      ),
    "videoNote",
  ).catch((err) => {
    throw wrapChatNotFound(err);
