...
vi.mock("./chrome.js", () => ({
  resolveaeonsageUserDataDir: vi.fn(() => "/tmp/aeonsage-test/aeonsage/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveaeonsageUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "aeonsage-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveaeonsageUserDataDir).mockReturnValue(userDataDir);
