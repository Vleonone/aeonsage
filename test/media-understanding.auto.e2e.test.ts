...
import type { AeonSageConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("aeonsage-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("aeonsage-bin-sherpa-");
      const modelDir = await makeTempDir("aeonsage-sherpa-model-");
      ...
      const cfg: AeonSageConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("aeonsage-bin-whispercpp-");
      const modelDir = await makeTempDir("aeonsage-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("aeonsage-bin-gemini-");
      ...
      const cfg: AeonSageConfig = { tools: { media: { image: {} } } };
