...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.resolve(root, entry.name, ".aeonsage");
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".aeonsage");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.aeonsage out of git; it contains credentials and session history.",
