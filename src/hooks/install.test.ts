...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `aeonsage-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@aeonsage/zip-hooks",
        version: "0.0.1",
        aeonsage: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@aeonsage/tar-hooks",
        version: "0.0.1",
        aeonsage: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
