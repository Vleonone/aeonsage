...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "aeonsage doctor",
        managerScriptArgs(manager, "aeonsage", ["doctor", "--non-interactive"]),
        gitRoot,
        { AEONSAGE_UPDATE_IN_PROGRESS: "1" },
      ),
    );
