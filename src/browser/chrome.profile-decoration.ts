...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".aeonsage-profile-decorated");
}
 ...
 */
export function decorateAeonSageProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AEONSAGE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AEONSAGE_BROWSER_COLOR).toUpperCase();
