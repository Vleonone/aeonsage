...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    const indexedLabel =
      totalFiles === null
        ? `${status.files}/? files 路 ${status.chunks} chunks`
        : `${status.files}/${totalFiles} files 路 ${status.chunks} chunks`;
    if (opts.index) {
    ...
    if (status.sourceCounts?.length) {
      ...
      for (const entry of status.sourceCounts) {
        ...
        const counts =
          total === null
            ? `${entry.files}/? files 路 ${entry.chunks} chunks`
            : `${entry.files}/${total} files 路 ${entry.chunks} chunks`;
        lines.push(`  ${accent(entry.source)} ${muted("路")} ${muted(counts)}`);
      }
    }
    if (status.fallback) {
...
export function registerMemoryCli(program: Command) {
  const memory = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.aeonsage.org/cli/memory")}\n`,
    );
          ...
          run: async (manager) => {
            try {
              ...
              const buildLabel = () => {
                ...
                const eta = formatEta();
                return eta
                  ? `${lastLabel} 路 elapsed ${elapsed} 路 eta ${eta}`
                  : `${lastLabel} 路 elapsed ${elapsed}`;
              };
              await withProgressTotals(
                {
                  label: "Indexing memory...",
