...
import { __testing, createAeonSageCodingTools } from "./pi-tools.js";
...
describe("createAeonSageCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createAeonSageTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAeonSageCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aeonsage-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "aeonsage-outside.txt");
