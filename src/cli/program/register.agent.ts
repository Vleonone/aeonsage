...
${formatHelpExamples([
          ['aeonsage agent --to +18888888888 --message "status update"', "Start a new session."],
          ['aeonsage agent --agent ops --message "Summarize logs"', "Use a specific agent."],
          [
            'aeonsage agent --session-id 1234 --message "Summarize inbox" --thinking medium',
            "Target a session with explicit thinking level.",
          ],
          [
            'aeonsage agent --to +18888888888 --message "Trace logs" --verbose on --json',
            "Enable verbose logging and JSON output.",
          ],
          ['aeonsage agent --to +18888888888 --message "Summon reply" --deliver', "Deliver reply."],
          [
            'aeonsage agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
            "Send reply to a different channel/target.",
          ],
        ])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.aeonsage.org/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.aeonsage.org/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
          ['aeonsage agents set-identity --agent main --name "AeonSage" --emoji "✨"', "Set name + emoji."],
          ["aeonsage agents set-identity --agent main --avatar avatars/aeonsage.png", "Set avatar path."],
          ["aeonsage agents set-identity --workspace ~/aeonsage --from-identity", "Load from IDENTITY.md."],
          [
            "aeonsage agents set-identity --identity-file ~/aeonsage/IDENTITY.md --agent main",
