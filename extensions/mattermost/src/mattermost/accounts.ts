...
export function listMattermostAccountIds(cfg: AeonSageConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: AeonSageConfig): string {
...
export function listEnabledMattermostAccounts(cfg: AeonSageConfig): ResolvedMattermostAccount[] {
