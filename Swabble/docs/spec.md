...
- Wake word gating (default "aeonsage" plus aliases) with bypass flag `--no-wake`.
...
- CLI implemented with Commander (SwiftPM package `Vleonone/Commander`); core types are available via the SwiftPM library product `Swabble` for embedding.
