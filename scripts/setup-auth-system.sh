#!/bin/bash
# Setup AeonSage Auth Management System
...
echo "=== AeonSage Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'aeonsage-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AeonSage message: Send warning via AeonSage itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/aeonsage-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/aeonsage-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/aeonsage-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now aeonsage-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/AeonSageAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/AeonSageAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/AeonSage*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (AeonSageAuth) shows status and opens auth URL if needed."
echo "The full widget (AeonSageAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status aeonsage-auth-monitor.timer"
echo ""
echo "Quick commands:"
