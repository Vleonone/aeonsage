...
export function createGatewayReloadHandlers(params: {
  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.AEONSAGE_SKIP_GMAIL_WATCHER)) {
        try {
        ...
        params.logHooks.info("skipping gmail watcher restart (AEONSAGE_SKIP_GMAIL_WATCHER=1)");
      }
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.AEONSAGE_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.AEONSAGE_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (AEONSAGE_SKIP_CHANNELS=1 or AEONSAGE_SKIP_PROVIDERS=1)",
        );
      } else {
    ...
    setCommandLaneConcurrency(CommandLane.Main, await resolveAgentMaxConcurrent(nextConfig));
    setCommandLaneConcurrency(CommandLane.Subagent, await resolveSubagentMaxConcurrent(nextConfig));
