...
const SANDBOX_EXAMPLES = {
  main: [
    ["aeonsage sandbox list", "List all sandbox containers."],
    ["aeonsage sandbox list --browser", "List only browser containers."],
    ["aeonsage sandbox recreate --all", "Recreate all containers."],
    ["aeonsage sandbox recreate --session main", "Recreate a specific session."],
    ["aeonsage sandbox recreate --agent mybot", "Recreate agent containers."],
    ["aeonsage sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["aeonsage sandbox list", "List all sandbox containers."],
    ["aeonsage sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["aeonsage sandbox recreate --all", "Recreate all containers."],
    ["aeonsage sandbox recreate --session main", "Recreate a specific session."],
    ["aeonsage sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["aeonsage sandbox recreate --browser --all", "Recreate only browser containers."],
    ["aeonsage sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["aeonsage sandbox explain", "Show effective sandbox config."],
    ["aeonsage sandbox explain --session agent:main:main", "Explain a specific session."],
    ["aeonsage sandbox explain --agent work", "Explain an agent sandbox."],
    ["aeonsage sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.aeonsage.org/cli/sandbox")}\n`,
    )
    .action(() => {
