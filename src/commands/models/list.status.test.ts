...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAeonSageAgentDir: vi.fn().mockReturnValue("/tmp/aeonsage-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAeonSageAgentDir: mocks.resolveAeonSageAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/aeonsage-agent/auth-profiles.json");
