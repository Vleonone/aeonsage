...
export async function createTelegramRunnerOptions(cfg: AeonSageConfig): Promise<RunOptions<unknown>> {
  return {
    sink: {
      concurrency: await resolveAgentMaxConcurrent(cfg),
    },
    runner: {
      fetch: {
...
export async function monitorTelegramProvider(opts: MonitorTelegramOpts = {}) {
  ...
  while (!opts.abortSignal?.aborted) {
    const runner = run(bot, await createTelegramRunnerOptions(cfg));
    const stopOnAbort = () => {
