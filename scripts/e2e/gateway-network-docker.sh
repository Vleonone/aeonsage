...
IMAGE_NAME="aeonsage-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="aeonsage-net-e2e-$$"
GW_NAME="aeonsage-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AEONSAGE_GATEWAY_TOKEN=$TOKEN" \
	  -e "AEONSAGE_SKIP_CHANNELS=1" \
	  -e "AEONSAGE_SKIP_GMAIL_WATCHER=1" \
	  -e "AEONSAGE_SKIP_CRON=1" \
	  -e "AEONSAGE_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
