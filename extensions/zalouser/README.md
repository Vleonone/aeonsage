...
AeonSage extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
aeonsage channels login --channel zalouser
...
```bash
aeonsage message send --channel zalouser --target <threadId> --message "Hello from AeonSage!"
```
...
```bash
aeonsage channels login --channel zalouser              # Login via QR
aeonsage channels login --channel zalouser --account work
aeonsage channels status --probe
aeonsage channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
aeonsage directory self --channel zalouser
aeonsage directory peers list --channel zalouser --query "name"
aeonsage directory groups list --channel zalouser --query "work"
aeonsage directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
aeonsage message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
aeonsage message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `aeonsage channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
aeonsage channels login --channel zalouser --account work
aeonsage message send --channel zalouser --account work --target <id> --message "Hello"
