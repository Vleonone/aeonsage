...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "AEONSAGE_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("AEONSAGE_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv AEONSAGE_GATEWAY_TOKEN");
    expect(message).not.toContain("AEONSAGE_GATEWAY_PASSWORD");
  });
