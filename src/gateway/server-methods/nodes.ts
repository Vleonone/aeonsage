...
export const nodeHandlers: GatewayRequestHandlers = {
  ...
  "node.list": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      // [NEW] Merge Remote Gateways
      if (context.remoteGatewayRegistry) {
        context.remoteGatewayRegistry.list().forEach(r => {
          // Treat as connected node for listing purposes
          if (!connectedById.has(r.id)) {
            // @ts-ignore
            connectedById.set(r.id, {
              nodeId: r.id,
              displayName: `Remote: ${r.url}`, // Simple display name
              connectedAtMs: r.connectedAt,
              caps: ["gateway"], // Assume gateway capability
              commands: ["agent.run"], // Assume basic commands for now
              remoteIp: r.url,
  ...
  "node.invoke": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      if (!nodeSession) {
        // [NEW] Check remote registry
        if (context.remoteGatewayRegistry) {
          const remoteSession = context.remoteGatewayRegistry.get(nodeId);
          if (remoteSession) {
            const result = await context.remoteGatewayRegistry.invoke({
              nodeId,
              command,
              params: p.params,
            ...
            if (result.ok) {
              ...
              respond(
                ...
                errorShape(
                  ...
                  result.error?.message || "Remote gateway error",
                  { details: { code: result.error?.code || "REMOTE_ERROR" } }
                )
              );
            }
            return;
  ...
  "node.connect": async ({ params, respond, context }) => {
    const { url } = params as { url?: string };
    if (url && context.remoteGatewayRegistry) {
      const result = await context.remoteGatewayRegistry.connect(url);
      if (result.ok) {
        respond(true, { ok: true, nodeId: result.nodeId, message: "Connected to remote gateway" });
      } else {
        respond(false, undefined, { code: "CONNECT_FAILED", message: result.error || "Unknown error" });
      }
      return;
    }

    // Stub implementation for UI method alignment
    // In WS architecture, connections are initiated by the node, not the server.
    // We return success to indicate the request was received, even if no active action is taken.
    respond(true, { ok: true, message: "Node connection request acknowledged" }, undefined);
  },
  "node.disconnect": async ({ params, respond, context }) => {
    ...
    if (node) {
      try {
        node.client.socket.close();
        respond(true, { ok: true, disconnected: true }, undefined);
      } catch (err: any) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.UNAVAILABLE, err?.message ?? "failed to close socket"),
        );
      }
    } else if (context.remoteGatewayRegistry && context.remoteGatewayRegistry.get(nodeId)) {
      await context.remoteGatewayRegistry.disconnect(nodeId);
      respond(true, { ok: true, disconnected: true }, undefined);
    } else {
      respond(true, { ok: true, disconnected: false, message: "node not active" }, undefined);
    }
  },
};
