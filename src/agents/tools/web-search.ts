...
type WebSearchConfig = NonNullable<AeonSageConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AeonSageConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("aeonsage configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.aeonsage.org/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://aeonsage.org",
      "X-Title": "AeonSage Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
