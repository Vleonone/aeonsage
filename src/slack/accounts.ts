...
export function listSlackAccountIds(cfg: AeonSageConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: AeonSageConfig): string {
...
function mergeSlackAccountConfig(cfg: AeonSageConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: AeonSageConfig): ResolvedSlackAccount[] {
