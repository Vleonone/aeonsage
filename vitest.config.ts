...
export default defineConfig({
  resolve: {
    alias: {
      "aeonsage/plugin-sdk": path.join(repoRoot, "src", "plugin-sdk", "index.ts"),
    },
  },
  test: {
    testTimeout: 120_000,
    hookTimeout: isWindows ? 180_000 : 120_000,
    // Windows + pnpm + forks = "Worker exited unexpectedly" crashes
    // Solution: threads pool on Windows only; Linux/macOS keeps forks
    // Windows optimization: Use threads (safer for node primitives) and allow concurrency
    pool: isWindows ? "threads" : "forks",
    // Unlock parallelism on Windows (half of available cores is usually safe/fast)
    maxWorkers: isWindows ? Math.floor(os.cpus().length / 2) : (isCI ? ciWorkers : localWorkers),
    isolate: true,
    include: [
      "src/**/*.test.ts",
      "extensions/**/*.test.ts",
