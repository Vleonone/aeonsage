#!/usr/bin/env bash
# Reset AeonSage like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${AEONSAGE_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="AeonSage.app/Contents/MacOS/AeonSage"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/AeonSage"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/AeonSage"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/AeonSage"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.aeonsage.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/aeonsage-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${AEONSAGE_RESTART_LOG:-/tmp/aeonsage-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${AEONSAGE_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.aeonsage/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  AEONSAGE_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      ...
      log "  rm ~/.aeonsage/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.aeonsage.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing AeonSage instances"
kill_all_aeonsage
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product AeonSage"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/AeonSage.app" ]]; then
    APP_BUNDLE="/Applications/AeonSage.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/AeonSage.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/AeonSage.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/AeonSage.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set AEONSAGE_APP_BUNDLE to your installed AeonSage.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".aeonsage", "aeonsage.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: AeonSage is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/com.aeonsage.gateway.plist' | head -n 40 || true"
fi
