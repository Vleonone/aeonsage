...
import { discoverAeonSagePlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AeonSagePluginDefinition;
  register?: AeonSagePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AeonSagePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AeonSagePluginDefinition;
...
export function loadAeonSagePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAeonSagePlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "aeonsage/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AeonSagePluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AeonSagePluginModule;
    } catch (err) {
