...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("AEONSAGE_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.AEONSAGE_STATE_DIR = path.join(base, ".aeonsage");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "aeonsage-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".aeonsage", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
