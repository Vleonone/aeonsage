...
// ===== Budget Control for Autonomous Subagent Management =====
...
export type SubagentBudget = {
  ...
  spawnCount: number;
  runningCount: number;
  efficiency: number; // Output quality / Token cost ratio
  lastResetAt: number;
};

const subagentBudgets = new Map<string, SubagentBudget>();

function getBudgetKey(requesterSessionKey: string): string {
  return requesterSessionKey.trim();
}

function resetBudgetIfNeeded(budget: SubagentBudget): void {
  const now = Date.now();
  const oneDay = 24 * 60 * 60 * 1000;
  if (now - budget.lastResetAt > oneDay) {
    budget.currentTokenUsage = 0;
    budget.spawnCount = 0;
    budget.lastResetAt = now;
  }
}

export function getOrCreateSubagentBudget(
  requesterSessionKey: string,
  dailyTokenLimit = 100000,
): SubagentBudget {
  const key = getBudgetKey(requesterSessionKey);
  let budget = subagentBudgets.get(key);
  if (!budget) {
    budget = {
      dailyTokenLimit,
      currentTokenUsage: 0,
    ...
    subagentBudgets.set(key, budget);
  }
  resetBudgetIfNeeded(budget);
  return budget;
}

export function updateSubagentBudget(
  requesterSessionKey: string,
  updates: Partial<Pick<SubagentBudget, "currentTokenUsage" | "runningCount" | "efficiency">>,
): void {
  const budget = getOrCreateSubagentBudget(requesterSessionKey);
  if (updates.currentTokenUsage !== undefined) {
    budget.currentTokenUsage = Math.max(0, updates.currentTokenUsage);
  }
  if (updates.runningCount !== undefined) {
    budget.runningCount = Math.max(0, updates.runningCount);
  }
  if (updates.efficiency !== undefined) {
    budget.efficiency = Math.max(0, updates.efficiency);
  }
}

export function incrementSubagentSpawnCount(requesterSessionKey: string): void {
  const budget = getOrCreateSubagentBudget(requesterSessionKey);
  budget.spawnCount++;
  budget.runningCount++;
}

export function decrementSubagentRunningCount(requesterSessionKey: string): void {
  const budget = getOrCreateSubagentBudget(requesterSessionKey);
  budget.runningCount = Math.max(0, budget.runningCount - 1);
}

export function getBudgetStatus(requesterSessionKey: string): {
  ...
  const budget = getOrCreateSubagentBudget(requesterSessionKey);
  const remainingTokens = Math.max(0, budget.dailyTokenLimit - budget.currentTokenUsage);
  const utilizationPercent = budget.dailyTokenLimit > 0
    ? (budget.currentTokenUsage / budget.dailyTokenLimit) * 100
    : 0;
  // Can spawn if under 90% budget and not exceeding running limit (10)
  const canSpawn = utilizationPercent < 90 && budget.runningCount < 10;
...
export function getSubagentBudgetForRequester(requesterSessionKey: string): SubagentBudget | null {
  const key = getBudgetKey(requesterSessionKey);
