...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/aeonsage/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AeonSage Re-Auth" -c "After getting key, run: ssh $SERVER '~/aeonsage/scripts/mobile-reauth.sh'" --id aeonsage-auth
        ;;
    *)
