...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("aeonsage node install"), formatCliCommand("aeonsage node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("aeonsage node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AEONSAGE_LOG_PREFIX: baseEnv.AEONSAGE_LOG_PREFIX ?? "node",
