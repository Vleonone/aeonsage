...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AEONSAGE_GATEWAY_PORT).toBe("18789");
    expect(env.AEONSAGE_GATEWAY_TOKEN).toBe("secret");
    expect(env.AEONSAGE_SERVICE_MARKER).toBe("aeonsage");
    expect(env.AEONSAGE_SERVICE_KIND).toBe("gateway");
    expect(typeof env.AEONSAGE_SERVICE_VERSION).toBe("string");
    expect(env.AEONSAGE_SYSTEMD_UNIT).toBe("aeonsage-gateway.service");
    if (process.platform === "darwin") {
      expect(env.AEONSAGE_LAUNCHD_LABEL).toBe("com.aeonsage.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AEONSAGE_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AEONSAGE_SYSTEMD_UNIT).toBe("aeonsage-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.AEONSAGE_LAUNCHD_LABEL).toBe("com.aeonsage.work");
    }
  });
});
