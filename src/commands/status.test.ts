...
beforeAll(() => {
  previousProfile = process.env.AEONSAGE_PROFILE;
  process.env.AEONSAGE_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.AEONSAGE_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/aeonsage-root.js", () => ({
  resolveAeonSagePackageRoot: vi.fn().mockResolvedValue("/tmp/aeonsage"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/aeonsage/pnpm-lock.yaml",
      markerPath: "/tmp/aeonsage/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.aeonsage.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.aeonsage.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("AeonSage status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("aeonsage status --all") ||
          l.includes("aeonsage --profile isolated status --all") ||
          l.includes("aeonsage status --all") ||
          l.includes("aeonsage --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.AEONSAGE_GATEWAY_TOKEN = "abcd1234";
    try {
