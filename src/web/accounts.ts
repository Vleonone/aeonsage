...
import { sanitizeId } from "../security/path-sanitizer.js";
...
function listConfiguredAccountIds(cfg: AeonSageConfig): string[] {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listWhatsAppAuthDirs(cfg: AeonSageConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: AeonSageConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: AeonSageConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: AeonSageConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: AeonSageConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
  // SECURITY: Sanitize accountId to prevent path traversal attacks (#4610)
  const rawId = params.accountId.trim() || DEFAULT_ACCOUNT_ID;
  const accountId = rawId === DEFAULT_ACCOUNT_ID ? rawId : sanitizeId(rawId, "accountId");
...
export function resolveWhatsAppAccount(params: {
  ...
  // SECURITY: Sanitize accountId to prevent path traversal attacks (#4610)
  const rawAccountId = params.accountId?.trim() || resolveDefaultWhatsAppAccountId(params.cfg);
  const accountId =
    rawAccountId === DEFAULT_ACCOUNT_ID ? rawAccountId : sanitizeId(rawAccountId, "accountId");
...
export function listEnabledWhatsAppAccounts(cfg: AeonSageConfig): ResolvedWhatsAppAccount[] {
