...
import { createAeonSageTools } from "../agents/aeonsage-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.AEONSAGE_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.AEONSAGE_GATEWAY_PORT = String(gatewayPort);
  process.env.AEONSAGE_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.AEONSAGE_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createAeonSageTools().find((candidate) => candidate.name === "sessions_send");
