...
`aeonsage.invoke --each`) without adding new AeonSage code per workflow.
...
This extension depends on AeonSage internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AeonSage extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.aeonsage/extensions` as a standalone plugin directory.
