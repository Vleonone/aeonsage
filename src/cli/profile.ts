...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.aeonsage${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.AEONSAGE_PROFILE = profile;

  const stateDir = env.AEONSAGE_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.AEONSAGE_STATE_DIR?.trim()) env.AEONSAGE_STATE_DIR = stateDir;

  if (!env.AEONSAGE_CONFIG_PATH?.trim()) {
    env.AEONSAGE_CONFIG_PATH = path.join(stateDir, "aeonsage.json");
  }

  if (profile === "dev" && !env.AEONSAGE_GATEWAY_PORT?.trim()) {
    env.AEONSAGE_GATEWAY_PORT = "19001";
  }
}
