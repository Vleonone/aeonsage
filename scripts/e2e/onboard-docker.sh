...
IMAGE_NAME="aeonsage-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/aeonsage-bin:$PATH"
  mkdir -p /tmp/aeonsage-bin
  cat > /tmp/aeonsage-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/aeonsage-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/aeonsage-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/aeonsage-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/aeonsage-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/aeonsage"
    config_path="$HOME/.aeonsage/aeonsage.json"
    sessions_dir="$HOME/.aeonsage/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.aeonsage/aeonsage.json" <<'"'"'JSON'"'"'
{
